...
namespace PiSubmarine::Drone::Server
{
    namespace
    {
        ...
        public:
            StartupVideoSubscriber(
                ...
                , m_Logger(loggerFactory.CreateLogger("Drone.Server.StartupVideoSubscriber"))
            {
            }
            ...
            void Stop() noexcept
            {
                ...
                if (!unsubscribeResult.has_value())
                {
                    SPDLOG_LOGGER_WARN(
                        m_Logger,
                        "Failed to remove startup video subscription for lease {}.",
                        leaseId.Value);
                }
    ...
    public:
        Impl(const Config& config, Dependencies dependencies)
            ...
            , VideoController(config.VideoController, m_LoggerFactory, LeaseManager, LeaseManager)
            , VideoSubscriptionServer(VideoController, m_LoggerFactory)
            , StartupSubscriber(
            ...
            , DepthTelemetrySerializer(dependencies.DepthTelemetryProvider)
            , LampTelemetrySerializer(dependencies.LampTelemetryProvider)
            , BallastMotorTelemetrySerializer(dependencies.BallastMotorTelemetryProvider)
            , FrontLeftMotorTelemetrySerializer(dependencies.FrontLeftMotorTelemetryProvider)
            , FrontRightMotorTelemetrySerializer(dependencies.FrontRightMotorTelemetryProvider)
            , ProximityTelemetrySerializer(dependencies.ProximityTelemetryProvider)
            , RearLeftMotorTelemetrySerializer(dependencies.RearLeftMotorTelemetryProvider)
            , RearRightMotorTelemetrySerializer(dependencies.RearRightMotorTelemetryProvider)
            ...
            , HorizontalController(
                  dependencies.FrontLeftMotorController,
                  dependencies.FrontRightMotorController,
                  dependencies.RearLeftMotorController,
                  dependencies.RearRightMotorController)
            , VerticalController(
                  BallastController,
            ...
            , TelemetryServer(
                  ...
                  TelemetrySocket,
                  TelemetrySocket)
            , PlatformTickables(std::move(dependencies.PlatformTickables))
            ...
            , StartupVideoEnable(config.StartupVideoEnable)
        {
            ...
            ThrowIfError(TimeManager.AddTickable(VerticalController), "adding vertical controller to Time.Manager");
            ThrowIfError(TimeManager.AddTickable(BallastController), "adding ballast controller to Time.Manager");
            for (Time::ITickable& tickable : PlatformTickables)
            {
                ThrowIfError(TimeManager.AddTickable(tickable), "adding platform tickable to Time.Manager");
            }
        ...
        Ballast::Pid::Controller BallastController;
        ...
        Control::Horizontal::FourJetDifferential::Controller HorizontalController;
        Control::Vertical::Ballast::Controller VerticalController;
        Control::Gimbal::Servo::Controller GimbalController;
    ...
    Runtime::Runtime(const Config& config, Dependencies dependencies)
        : m_Impl(std::make_unique<Impl>(config, std::move(dependencies)))
    {
    }
    ...
    Error::Api::Result<void> Runtime::StartServices()
    {
        ...
        {
            const auto controlLeaseResult = m_Impl->LeaseManager.AcquireLease(
                Lease::Api::LeaseRequest{.Resource = MakeControlResourceId()});
    ...
    void Runtime::ThrowIfError(const Error::Api::Result<void>& result, const char* action)
    {
        if (!result.has_value())
        {
            throw std::runtime_error(std::string("Drone.Server runtime failed while ") + action);
        }
    }
}
